...
This step builds two current-condition spatial layers for each of the seven TBCMP counties using the input layers from Step 1. All processing is implemented in `R/funcs.R` and executed in `R/02_current_layers.R`.

## Overview

For each county, two layers are produced that together characterize the current state of the coastal landscape in terms of habitat type, protection status, and restoration potential:
...
2. Call `build_current_lyrs()` to produce the two output layers
